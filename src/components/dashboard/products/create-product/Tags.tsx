...
const Tags = () => {
  ...
  return (
    <div className="w-full flex flex-col gap-2">
        <Label htmlFor="tag-input">Tags<span className="text-[var(--h-text-gray)]">Optional</span></Label>
        <Controller
            control={control}
            name="tags"
            render={() => (
            <>
                ...
                <Input
                    ...
                    onKeyDown={handleEnterKey} // Optional: Enter से भी add कर सकते हो
                />
                <Button_Input_Absolute func={handleAddTag}>Add</Button_Input_Absolute>
                {errors?.tags && <span className="text-[12px] text-red-600">{errors?.tags?.message}</span>}
                </div>
                {tags.length > 0 && (
